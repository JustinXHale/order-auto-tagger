# Order auto-tagger (Shopify app)

This app tags **orders** when they are created: you define per-store **rules** (products + optional date window → tags). New orders that match get the tags so you can filter or export a subset in Shopify Admin. Rules are stored per shop in the app database; sessions use Prisma ([`prisma/schema.prisma`](prisma/schema.prisma)).
...
This project started from the Shopify [React Router app template](https://github.com/Shopify/shopify-app-template-react-router). The sections below are retained from the template.
